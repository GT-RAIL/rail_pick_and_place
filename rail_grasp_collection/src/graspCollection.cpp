...
void graspCollection::executePickup(const rail_grasp_collection::PickupGoalConstPtr& goal)
{
  ...
  */
  ...
  try { listener.lookupTransform("base_footprint", "/jaco_link_finger_1", ros::Time(0), finger1Transform); }
  catch (tf::TransformException ex) { ROS_ERROR("%s",ex.what()); }
  try { listener.lookupTransform("base_footprint", "/jaco_link_finger_2", ros::Time(0), finger2Transform); }
  catch (tf::TransformException ex) { ROS_ERROR("%s",ex.what()); }
  tf::StampedTransform rightFingerTransform;
  try { listener.lookupTransform("base_footprint", "/jaco_link_finger_3", ros::Time(0), finger3Transform); }
  catch (tf::TransformException ex) { ROS_ERROR("%s",ex.what()); }
